...
**Cursor ↔ Claude sync:** Полное зеркало правил из `.cursor/rules/`, живая память и обязательная **взаимная синхронизация** при правках — в **`CLAUDE.md`**. Меняя правила или `memory.mdc` / `context.mdc`, обновляй **`CLAUDE.md`** в той же сессии (и наоборот). Дублирующий блок конфигурации здесь держи согласованным с `CLAUDE.md`.
