...
<div class="space-y-6">
  <div>
    ...
    <p class="mt-2 text-sm text-foreground-muted">
      {m.oryxel_agent_memory_hint({ max: String(maxRows) })}
    </p>
  </div>
  ...
  <Card class="space-y-3 p-6">
    ...
    <Button type="button" size="sm" disabled={busy || !draft.trim() || items.length >= maxRows} onclick={addItem}>
      {m.oryxel_agent_memory_add()}
    </Button>
    {#if items.length >= maxRows}
      <p class="text-xs text-foreground-muted">{m.oryxel_agent_memory_limit({ max: String(maxRows) })}</p>
    {/if}
  </Card>
